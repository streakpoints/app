...
const gameSP = '0xfb86e23C71EcfD07AF371B290a453704F52B1f9A';
const tokenSP = '0x46DcBdF861e0EECfA81515C572FaD590D747F393';
          ...
          <tbody>
            {
              checkins.map(c => (
                <tr key={`${c.address}-${c.epoch}`}>
                  <td>
                    <a href={`https://polygonscan.com/token/${tokenSP}?a=${c.address}`} target='_blank'>{c.name || (`${c.address.substr(0, 6)}...${c.address.substr(-4)}`)}</a>
                    <br />
